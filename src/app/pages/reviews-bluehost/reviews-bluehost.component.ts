...
@Component({
  ...
  templateUrl: "./reviews-bluehost.component.html",
  styleUrls: ["./reviews-bluehost.component.css"],
  providers: [DatePipe]
})
export class AboutComponentbluehost implements OnInit, AfterViewChecked, ElementRef {
  ...
  getAllProducts() {
    this.api.getBluehost().subscribe({
      next: (res) => {
// console.log(res);
  ...
  handleNotify(eventData: Employee) {
    ...
    this.api.GetItemBluehost(this.vontedataid).subscribe(res => {
  ...
  onUpdate(): any {
    this.api.updateItembluehost(this.vontedataid, this.updateForm.value)
      .subscribe(() => {
      }, (err) => {
        console.log(err);
      });
  ...
  getReviewCount() {
    ...
    this.api.getBluehost().subscribe((response) => {
  ...
  addProduct() {
    if (this.productForm.valid) {
      this.api.AddBluehost(this.productForm.value).subscribe({
        next: (res) => {
  ...
  onSubmit(): any {
    if (this.productForm.valid) {
      this.api.AddBluehost(this.productForm.value)
        .subscribe(() => {
  ...
  getFriends() {
    ...
    this.api.getBluehost()
      .subscribe((response) => {
        this.allUser = response;

        this.allUser.forEach((row: any) => {
          if (row.companyName == "bluehost") {
            this.allUser2 = this.allUser.filter(
              (product: { companyName: string }) =>
                product.companyName === "bluehost"
            ).length;
