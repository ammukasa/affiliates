...
@Component({
  ...
  templateUrl: "./reviews-dreamhost.component.html",
  styleUrls: ["./reviews-dreamhost.component.css"],
  providers: [DatePipe]
})


export class AboutComponentdreamhost implements OnInit, AfterViewChecked, ElementRef {
  ...
  getAllProducts() {
    this.api.getDreamhost().subscribe({
      next: (res) => {
// console.log(res);
  ...
  handleNotify(eventData: Employee) {
    ...
    this.api.GetItemDreamhost(this.vontedataid).subscribe(res => {
  ...
  onUpdate(): any {
    this.api.updateItemdreamhost(this.vontedataid, this.updateForm.value)
      .subscribe(() => {
      }, (err) => {
        console.log(err);
      });
  ...
  getReviewCount() {
    ...
    this.api.getDreamhost().subscribe((response) => {
  ...
  addProduct() {
    if (this.productForm.valid) {
      this.api.AddDreamhost(this.productForm.value).subscribe({
        next: (res) => {
  ...
  onSubmit(): any {
    if (this.productForm.valid) {
      this.api.AddDreamhost(this.productForm.value)
        .subscribe(() => {
  ...
  getFriends() {
    ...
    this.api.getDreamhost()
      .subscribe((response) => {
        this.allUser = response;

        this.allUser.forEach((row: any) => {
          if (row.companyName == "dreamhost") {
            this.allUser2 = this.allUser.filter(
              (product: { companyName: string }) =>
                product.companyName === "dreamhost"
            ).length;
